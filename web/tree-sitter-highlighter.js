(function () {
  ...
  async function languageFor(language) {
    ...
    if (!languages.has(language)) {
      languages.set(
        language,
        window.TreeSitter.Language.load(assetRoot + wasm),
      );
    }
  ...
  function kindFor(node, source) {
    ...
    if (/^(identifier|property_identifier|type_identifier)$/.test(type)) {
      ...
      if (parent && /(class|function|method|declaration|definition)/.test(parent)) {
        return 'definition';
      }
      return 'identifier';
    }
    if (node.childCount === 0) {
      ...
      if (/^(async|await|break|case|catch|class|const|continue|def|default|delete|do|else|enum|export|extends|finally|for|from|function|if|import|in|interface|let|new|return|static|switch|throw|try|var|while|with|yield)$/.test(token)) return 'keyword';
